  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between px-8 lg:px-32 font-sans text-black">
        ...
        <section className='my-8 lg:my-16 flex flex-col'>
            ...
            <div className='py-4 lg:py-20'>
              ...
              <p className='my-4 text-2xl lg:text-7xl xl:text-8xl'>{t('gameDetails2')}</p>
                <Image className='w-full lg:my-12 rounded-md' src='/images/gameplay-island.png' width={420} height={280} alt='gameplay'/>
            </div>
            {/* DETAILS 2 */}
            <div className='py-4'>
                <p className='my-4 text-2xl lg:text-7xl xl:text-8xl'>{t('gameDetails3')}</p>
                <Image className='w-full lg:my-12 rounded-md' src='/images/gameplay-home.png' width={420} height={280} alt='gameplay'/>
            {/* LEARN MORE */}
            <div className='py-4 flex flex-col items-center justify-center'>
              <p className='my-4 text-2xl lg:text-8xl text-center'>{t('gameDetails4')}</p>
              <Link className='hover:text-white hover:border-white text-xl bg-white bg-opacity-20 text-black border-2 border-black rounded-md my-4 py-2 px-3 text-center lg:w-[350px]' href="/about">{t('more')}</Link>
            </div>
            </div>
        </section>
        ...
        <section className='my-8 lg:mb-32 flex flex-col justify-evenly items-center'>
            ...
            <button className='hover:text-white hover:border-white bg-white bg-opacity-20 text-black border-2 border-black rounded-md py-2 px-3 lg:w-[350px] text-xl'onClick={()=>handleNewsletter()} >{t('sign up')}</button>  
        </section>
    </main>
  )
}
